...
internal struct JSON5Scanner {
    ...
    mutating func scan() throws -> JSONMap {
        ...
        if let char = try reader.consumeWhitespace(allowingEOF: true) {
            throw JSONError.unexpectedCharacter(context: mobs("after top-level value"), ascii: char, location: reader.sourceLocation)
        }

        return JSONMap(mapBuffer: partialMap.mapData, dataBuffer: reader.bytes)
    }
    ...
    mutating func scanArray() throws {
        ...
        ScanValues:
            ...
            case ._space, ._return, ._newline, ._tab:
                preconditionFailure(
                    "Expected that all white space is consumed",
                    file: "",
                    line: 189
                )
            case ._closebracket:
    ...
    mutating func scanKey() throws {
        ...
        default:
            ...
            guard unicodeScalar.isJSON5UnquotedKeyStartingCharacter else {
                throw JSONError.unexpectedCharacter(context: mobs("at beginning of JSON5 unquoted key"), ascii: firstChar, location: reader.sourceLocation)
            }
            try scanString(withQuote: nil)
        }
    }
    ...
    struct DocumentReader {
        ...
        mutating func readExpectedString(_ str: StaticString, typeDescriptor: String) throws {
            ...
            guard cmp == 0 else {
                ...
                throw JSONError.unexpectedCharacter(context: mobs("in expected \(typeDescriptor) value"), ascii: peek(offset: badOffset).unsafelyUnwrapped, location: sourceLocation(atOffset: badOffset))
            }
...
extension JSON5Scanner {
    ...
    private static func parseEscapeSequence(
        ...
        precondition(
            !jsonBytes.isEmpty,
            mobs("Scanning should have ensured that all escape sequences have a valid shape"),
            file: "",
            line: 887
        )
    ...
    static func validateLeadingZero(
        ...
        case UInt8(ascii: "x"), UInt8(ascii: "X"):
            ...
            guard firstHexDigitIndex <= jsonBytes.endIndex else {
                throw JSONError.unexpectedCharacter(context: mobs("in number"), ascii: jsonBytes[offset: 0], location: .sourceLocation(at: jsonBytes.startIndex, fullSource: fullSource))
            }
            let maybeHex = jsonBytes[unchecked: firstHexDigitIndex]
            guard maybeHex.isValidHexDigit else {
                throw JSONError.unexpectedCharacter(context: mobs("in number"), ascii: maybeHex, location: .sourceLocation(at: firstHexDigitIndex, fullSource: fullSource))
            }
    ...
    static func validateLeadingDecimal(
        ...
        guard case _asciiNumbers = nextByte else {
            throw JSONError.unexpectedCharacter(context: mobs("after '.' in number"), ascii: nextByte, location: .sourceLocation(at: jsonBytes.startIndex, fullSource: fullSource))
        }
    }
    ...
    static func prevalidateJSONNumber(
        ...
        case UInt8(ascii: "-"), UInt8(ascii: "+"):
            guard jsonBytes.count > 1 else {
                throw JSONError.unexpectedCharacter(context: mobs("at end of number"), ascii: jsonBytes[offset: 0], location: .sourceLocation(at: jsonBytes.startIndex, fullSource: fullSource))
            }
            ...
            case let byte: // default
                ...
                throw JSONError.unexpectedCharacter(context: mobs("after '\(String(UnicodeScalar(jsonBytes[offset: 0])))' in number"), ascii: byte, location: .sourceLocation(at: second, fullSource: fullSource))
            }
        case UInt8(ascii: "I"):
    ...
    static func validateNumber(
        ...
        let cmp = jsonBytes[index ..< endIndex].prefix(2).withUnsafePointer({ strncasecmp($0, "0x", $1) })
        if cmp == 0 {
            ...
            while index < endIndex {
                if jsonBytes[index].isValidHexDigit {
                    ...
                    return JSONError.unexpectedCharacter(context: mobs("in hex number"), ascii: jsonBytes[index], location: .sourceLocation(at: index, fullSource: fullSource))
                }
            }
            preconditionFailure(
...
extension BufferView where Element == UInt8 {
    internal func _decodeScalar() -> (Unicode.Scalar?, scalarLength: Int) {
        ...
        default:
            fatalError(
                mobs("Can't decode UTF-8 scalar with length \(len)"),
                file: "",
                line: 1307
            )
        }
